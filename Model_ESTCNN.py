...
class ESTCNN_moreblock(nn.Module):
    def __init__(self, n_classes, input_ch, input_time, batch_norm_alpha=0.1):
        super(ESTCNN_moreblock, self).__init__()
        self.batch_norm_alpha = batch_norm_alpha
        self.n_classes = n_classes
        ...
        """ Classifier """
        self.clf = nn.Sequential(nn.Linear(self.n_outputs, 1024),
                                 nn.ELU(),
                                 nn.Linear(1024, 50),
                                 nn.ELU(),
                                 nn.Linear(50, self.n_classes),
...
class ESTCNN_2moreblock(nn.Module):
    def __init__(self, n_classes, input_ch, input_time, batch_norm_alpha=0.1):
        super(ESTCNN_2moreblock, self).__init__()
        self.batch_norm_alpha = batch_norm_alpha
        self.n_classes = n_classes
        n_ch1, n_ch2, n_ch3, n_ch4, n_ch5 = 16, 32, 64, 128, 256
        ...
        self.convnet = nn.Sequential(
            ...
            nn.Conv2d(n_ch4, n_ch5, kernel_size=(1, 3), stride=1, padding="valid"),
            nn.ReLU(),
            nn.BatchNorm2d(n_ch5, momentum=self.batch_norm_alpha, affine=True, eps=1e-5),
            nn.Conv2d(n_ch5, n_ch5, kernel_size=(1, 3), stride=1, padding="valid"),
            nn.ReLU(),
            nn.BatchNorm2d(n_ch5, momentum=self.batch_norm_alpha, affine=True, eps=1e-5),
            nn.Conv2d(n_ch5, n_ch5, kernel_size=(1, 3), stride=1, padding="valid"),
            nn.ReLU(),
